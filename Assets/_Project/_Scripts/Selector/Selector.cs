...
namespace GridSelector
{
    ...
    public class Selector : MonoBehaviour
    {
        ...
        public void SetSelectorPosition(Vector2 position)
        {
            ...
            // Needs refactoring of item positioning before being able to do
        ...
        private void OnRotate()
        {
            ...
            SetSelectorSize(_grabbedItem.Size);
            SetSelectorPosition(ClampPositionInGrid(transform.localPosition, _grabbedItem));

            var gridPos = new Vector2Int((int)transform.localPosition.x, (int)transform.localPosition.y);
            _canPlace = _grid.CheckGrid(gridPos, _grabbedItem.Size);

            if (!_canPlace)
                SetSelectorColor(_unavailableSlotColor);
            else
                SetSelectorColor(_defaultSlotColor);
         ...
         private Vector3 CheckMovement(Vector2 movement)
        {
            ...
            return ClampPositionInGrid(targetPosition, _grabbedItem);
        }
        #endregion

        #region ---- SELECTOR ITEM DETECTION ----
        public void CheckSelectorSize()
        {
            var currentHoveredItem = DetectItems();
            ...
            if (currentHoveredItem != null && _hoveredItem != currentHoveredItem)
            {
                SetSelectorSize(currentHoveredItem.Size);
                SetSelectorPosition(currentHoveredItem.transform.localPosition);
                _hoveredItem = currentHoveredItem;
            }
            else if(currentHoveredItem == null)
            {
                SetSelectorSize(Vector2Int.one);
                _hoveredItem = null;
            }
        }
        ...
        private void GrabItem()
        {
            ...
            _grabbedItem.Hover(_grabbedItemOffset, transform);
        ...
        private void DropItem()
        {
            ...
            Vector2Int dropPosition = new Vector2Int((int)transform.localPosition.x, (int)transform.localPosition.y);
            _grabbedItem.Drop(dropPosition, _grid.ItemContainer);
